...
const extractSettings = (payload: unknown): DeepPartial<PreferenceSettings> | null => {
  ...
  if (
    "reader" in (payload as Record<string, unknown>) ||
    "editor" in (payload as Record<string, unknown>)
  ) {
...
const extractSources = (payload: unknown): Record<string, EffectiveSettingSource> => {
  if (!payload || typeof payload !== "object") return {};
  const raw = payload as Record<string, unknown>;
  const entries = Object.entries(raw).filter((entry): entry is [string, EffectiveSettingSource] => {
    const value = entry[1];
...
export const getWorkspaceSettings = async (workspaceId: string, config?: RequestConfig) => {
  const data = await unwrap<SettingsPayload | DeepPartial<PreferenceSettings>>(
...
export const clearWorkspaceSettings = async (workspaceId: string, config?: RequestConfig) => {
  await unwrap<void>(api.delete(`/workspaces/${workspaceId}/settings`, config));
};

export const getEffectiveSettings = async (workspaceId?: string | null, config?: RequestConfig) => {
