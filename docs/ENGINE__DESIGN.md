...
export interface APIAdapter {
  ...
  createDocument(
    doc: Omit<DocumentDTO, "docId" | "version">,
  ): Promise<DocumentDTO>;
  ...
  createBlock(
    docId: string,
    block: Omit<BlockDTO, "blockId">,
  ): Promise<BlockDTO>;
  updateBlock(
    docId: string,
    blockId: string,
    updates: Partial<BlockDTO>,
  ): Promise<BlockDTO>;
...
export class MockAdapter implements APIAdapter {
  ...
  private insertBlockIntoTree(
    ...
    for (const child of node.children || []) {
      if (this.insertBlockIntoTree(child, parentId, block, afterBlockId))
        return true;
    }
...
export class HttpAdapter implements APIAdapter {
  ...
  async getBlocks(docId: string, blockIds: string[]): Promise<BlockDTO[]> {
    return this.request<BlockDTO[]>(
      "POST",
      `/documents/${docId}/blocks/batch-get`,
      { blockIds },
    );
  }
  ...
  async updateBlock(
    ...
    return this.request<BlockDTO>("PATCH", `/blocks/${blockId}/content`, {
      docId,
      ...updates,
    });
  }
  ...
  async batchUpdate(
    ...
    return this.request<BatchResult>("POST", "/blocks/batch", {
      docId,
      operations,
    });
  }
  ...
  async getVersions(
    ...
    return this.request<DocumentVersion[]>(
      "GET",
      `/documents/${docId}/revisions?limit=${limit}`,
    );
  }

  async revertToVersion(docId: string, version: number): Promise<DocumentDTO> {
    return this.request<DocumentDTO>("POST", `/documents/${docId}/revert`, {
      version,
    });
  }
}
...
export class DocumentEngine {
  ...
  async createBlock(
    ...
    // 本地插入
    this.insertBlockLocally(
      doc,
      newBlock,
      parentId || doc.root.blockId,
      afterBlockId,
    );
...
export class VirtualScroller {
  ...
  private calculateLayout(): void {
    ...
    for (let i = 0; i < this.items.length; i++) {
      ...
      const height =
        this.itemHeights.get(block.blockId) || this.config.estimatedItemHeight;
